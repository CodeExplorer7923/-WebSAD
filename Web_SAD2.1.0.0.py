...
class ActualFunction(object):
    ...
    def clear_and_uninstall(self,shortcut_path):
...
>>>请选择功能：''')
        try:
            if func_select == '1':
                ...
                os.system('cls' if os.name == 'nt' else 'clear')
                self.clear_and_uninstall(shortcut_path=self.get_desktop_path()+r'\Web_SAD.lnk')
...
class IndexInstall(object):
    ...
    def install_and_configurate(self):
        ...
        if not os.path.exists(path=self.installed_path):
            self.create_shortcut(path=shortcut_path, target=os.getcwd()+R'\Web_SAD2.1.0.0.py', wDir=os.getcwd())  # *********************************
            with open(self.installed_path, 'w', encoding='utf-8')as f:
                json.dump(installed_record, f)
        if os.path.exists('./successful.mp3'):
            pygame.mixer.init()
